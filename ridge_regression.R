...
load()
prediction_DNN<-read.delim("processed_data/predictions/DNN_prediction.txt",header=F)
prediction_CNN<-read.delim("processed_data/predictions/CNN_prediction.txt",header=F)
dataset<-cbind(prediction_DNN,prediction_CNN,prediction_combine,prediction_logical,prediction_model,prediction_stem)
colnames(dataset)<-c("dnn","cnn","combine","logical","final","stem")
